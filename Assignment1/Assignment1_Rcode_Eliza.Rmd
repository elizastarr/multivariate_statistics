...
# No variables should be dropped from f1, f2
# f3's alpha is really low. Drop RDExp and NrEmployees
...
```
```{r}
# all chi-square tests fail
NrFactors <- 2:5
pvals <- rep(NA,times=length(NrFactors))
for(k in NrFactors){
  # Estimate k-factor model:
  Estimation <- factanal(no_outliers[,c(8:17)],factors=k,rotation="none")
  pvals[k] <- Estimation$PVAL
}
...
EFA.kfactors
```
1. Info1, Info2, Info3, Info4, Info5 - InfoIndustrySector
  - Sources within your firm or enterprise group
  - Suppliers of facilities, equipment, components, software and services
  - Customers or principals
  - Competitor or other enterprise in your industry sector
  - Consulting firms, private R&D business, commercial laboratories
2. Info6, Info7 - InfoResearchSector
  - Universities, universities of applied science or other higher education institutions
  - National or non-profit research institutions
3. Info8, Info9, Info10 - InfoOther
  - Conferences, fairs, exhibitions
  - Scientific periodicals and specialty publications
  - Associations and boards
...
```{r}
# This test shows that 5 factors is too many. Info 2, 5 and 8 move groupings
EFA.kfactors.1 <- factanal(no_outliers.Partition1[,c(8:17)], factors=3, rotation="varimax")
EFA.kfactors.2 <- factanal(no_outliers.Partition2[,c(8:17)], factors=3, rotation="varimax")
EFA.kfactors.1
EFA.kfactors.2
```
...
```{r}
EFA.3factors <- factanal(no_outliers[,c(8:17)], factors=3, rotation="varimax", scores="regression")
...
The Info varialbes are from the ordinal scale because the interpretation of each option can vary from person to person meaning that the difference between the variables is relative. However, the answer options do have a relative order.
...
The factors are not normally distributed. There is a significant difference between all Info variables and Innovation variables except for InfoResearchSector and LogisticsInnovation.
...
The Kendall's Tau test (non-parametric) rejects the null hypothesis of independence for all factors. However, the Kendall's tau values are not large so correlation is not strong.

```{r}
# make heatmap
cormat <- round(cor(final.data[,c(1,8:10)]),2)
cormat
...
```
```{r}
# Info variables not normally distributed
